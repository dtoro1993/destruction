...
public class PlayerAttacks : MonoBehaviour {
		...
		if (Input.GetKeyDown ("e")) {
			...
			//else if we are carrying an object. THROW IT 
				...
				if (carry.tag == "Enemy") {
                    ...
                    //sounds will randomize between 5 sounds.
                    SoundManager.instance.RandomizeSfx(EnemyThrow1, EnemyThrow2, EnemyThrow3, EnemyThrow4, EnemyThrow5, EnemyThrow6);
	...
	GameObject LiftScan(){
		...
		if (Physics.SphereCast (ray, radius, out hit, length,PickUpMask)) {
			...
			if (hit.collider.gameObject.tag == "Enemy" || hit.collider.gameObject.tag == "Throw") {
